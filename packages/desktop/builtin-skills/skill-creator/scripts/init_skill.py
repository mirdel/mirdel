...
In this app, new skills are always created under the public skills directory.
When run via the app, SKILLS_PUBLIC_ROOT is set; otherwise defaults to current dir.
...
Examples:
    ...
    init_skill.py list-desktop-files
"""
...
def _validate_skill_name(skill_name):
    """Check kebab-case, length; return (ok, error_message)."""
    if not skill_name or not isinstance(skill_name, str):
        return False, "Skill name is required"
    s = skill_name.strip()
    if not re.match(r"^[a-z0-9-]+$", s):
        return False, "Skill name must be kebab-case (lowercase letters, digits, hyphens only)"
    if s.startswith("-") or s.endswith("-") or "--" in s:
        return False, "Skill name cannot start/end with hyphen or contain consecutive hyphens"
    if len(s) > 64:
        return False, f"Skill name too long (max 64 characters, got {len(s)})"
    return True, s
...
def init_skill(skill_name):
    ...
    Creates under SKILLS_PUBLIC_ROOT if set (this app), else current directory.
    ...
    """
    ok, msg = _validate_skill_name(skill_name)
    if not ok:
        print(f"❌ Error: {msg}")
        return None
    skill_name = msg

    base = os.environ.get("SKILLS_PUBLIC_ROOT", ".")
    skill_dir = Path(base).resolve() / skill_name
...
def main():
    if len(sys.argv) != 2:
        ...
        print("\nSkill name: kebab-case (e.g. my-data-analyzer), max 64 characters.")
        print("New skill is created under the app's public skills directory.")
        print("\nExamples:")
        print("  init_skill.py my-new-skill")
        print("  init_skill.py list-desktop-files")
        sys.exit(1)
