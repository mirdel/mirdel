...
type ComposerConfig = {
  activeTask: VideoTaskType;
  byTask: {
    generate: ComposerTaskConfig;
  };
};

export type VideoWorkspaceRecord = {
  id: string;
  name: string;
  createdAt: number;
  updatedAt: number;
...
function sanitizeComposerTaskConfig(input: Partial<ComposerTaskConfig> | undefined): ComposerTaskConfig {
  return {
    ...
    duration: typeof input?.duration === "number" && Number.isFinite(input.duration) ? Math.floor(input.duration) : null,
    fps: typeof input?.fps === "number" && Number.isFinite(input.fps) ? Math.floor(input.fps) : null,
    ...
    referenceImages: Array.isArray(input?.referenceImages) ? input.referenceImages.filter((item): item is { id?: string; name?: string; url: string } => (
      !!item && typeof item === "object" && typeof item.url === "string" && item.url.trim().length > 0
    )) : [],
...
function normalizeGenerationParams(input: Record<string, any> | undefined): Record<string, any> {
  ...
  return {
    ...
    duration: typeof params.duration === "number" && Number.isFinite(params.duration) ? Math.floor(params.duration) : null,
    fps: typeof params.fps === "number" && Number.isFinite(params.fps) ? Math.floor(params.fps) : null,
...
function extFromMediaType(mediaType: string) {
  ...
  if (mediaType === "video/mpeg") return "mpeg";
  return "bin";
}
...
export function listVideoWorkspaces(): VideoWorkspaceRecord[] {
  const db = getDb();
  const rows = db
    .prepare("SELECT id, name, lastComposerJson, createdAt, updatedAt FROM video_workspaces ORDER BY updatedAt DESC")
...
export function createVideoWorkspace(input: { name?: string; lastComposer?: Partial<ComposerConfig> }) {
  const db = getDb();
  const ts = now();
  const workspace: VideoWorkspaceRecord = {
    id: uid("workspace"),
    name: (input.name || "").trim() || tMain("video.workspaceDefaultName"),
    createdAt: ts,
    updatedAt: ts,
...
export function renameVideoWorkspace(input: { workspaceId: string; name: string }) {
  const db = getDb();
  const ts = now();
  db.prepare("UPDATE video_workspaces SET name = ?, updatedAt = ? WHERE id = ?").run(
    input.name.trim() || tMain("video.workspaceDefaultName"),
    ts,
    input.workspaceId
  );
  return { ok: true };
...
export async function deleteVideoWorkspace(input: { workspaceId: string }) {
  const db = getDb();
  const deleteTx = db.transaction((workspaceId: string) => {
    const assetRows = db
      .prepare(
        `SELECT DISTINCT l.assetId AS assetId, a.filePath AS filePath
         ...
         JOIN video_generation_runs r ON r.id = l.ownerId
         JOIN video_generations g ON g.id = r.generationId
         WHERE l.ownerType = 'video_generation_run'
           AND g.workspaceId = ?`
      )
    ...
    db.prepare(
      `DELETE FROM asset_links
       WHERE ownerType = 'video_generation_run'
         AND ownerId IN (
           SELECT r.id
           FROM video_generation_runs r
           JOIN video_generations g ON g.id = r.generationId
           WHERE g.workspaceId = ?
         )`
    ).run(workspaceId);
    db.prepare("DELETE FROM video_generation_runs WHERE generationId IN (SELECT id FROM video_generations WHERE workspaceId = ?)").run(workspaceId);
    db.prepare("DELETE FROM video_generations WHERE workspaceId = ?").run(workspaceId);
    db.prepare("DELETE FROM video_workspaces WHERE id = ?").run(workspaceId);
...
export function updateVideoWorkspaceLastComposer(input: { workspaceId: string; lastComposer: Partial<ComposerConfig> }) {
  const db = getDb();
  const current = db.prepare("SELECT lastComposerJson FROM video_workspaces WHERE id = ?").get(input.workspaceId) as { lastComposerJson: string | null } | undefined;
  ...
  db.prepare("UPDATE video_workspaces SET lastComposerJson = ? WHERE id = ?").run(
    nextJson,
    input.workspaceId
  );
  return { ok: true };
...
function listRunsByWorkspace(workspaceId: string): VideoGenerationRunRecord[] {
  const db = getDb();
  const rows = db
    .prepare(
...
export function listVideoGenerationsByWorkspace(input: { workspaceId: string }): VideoGenerationRecord[] {
  const db = getDb();
  const generationRows = db
    .prepare("SELECT id, workspaceId, createdAt, updatedAt FROM video_generations WHERE workspaceId = ? ORDER BY createdAt ASC")
  ...
  const runsByGeneration = new Map<string, VideoGenerationRunRecord[]>();
  const runById = new Map<string, VideoGenerationRunRecord>();
  ...
  const assetsByGeneration = new Map<string, VideoAssetRecord[]>();
  for (const row of assetRows) {
  ...
  return generationRows.map((row) => {
    ...
    return {
      ...
      updatedAt: row.updatedAt,
      runs: generationRuns,
      videos: assetsByGeneration.get(row.id) || [],
    };
  });
}

export function createVideoGeneration(input: {
  workspaceId: string;
  prompt?: string;
  status?: GenerationStatus;
  selectedModel?: string;
  ...
  const db = getDb();
  const workspace = db.prepare("SELECT id FROM video_workspaces WHERE id = ?").get(input.workspaceId) as { id: string } | undefined;
  if (!workspace) {
    throw new Error("video workspace not found");
  }
  const ts = now();
  ...
  db.prepare(
    `INSERT INTO video_generations (id, workspaceId, prompt, status, selectedModel, paramsJson, errorMessage, createdAt, updatedAt)
     VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)`
  ).run(
    generation.id,
  ...
  db.prepare("UPDATE video_workspaces SET updatedAt = ? WHERE id = ?").run(ts, input.workspaceId);
  return generation;
}

export function createVideoGenerationRun(input: {
  generationId: string;
  prompt: string;
  status: GenerationStatus;
  selectedModel: string;
  ...
  const db = getDb();
  const ts = now();
  const run: VideoGenerationRunRecord = {
    id: uid("run"),
  ...
  db.prepare(
    `INSERT INTO video_generation_runs (id, generationId, prompt, status, selectedModel, paramsJson, errorMessage, warningMessage, startedAt, completedAt, createdAt, updatedAt)
  ...
  db.prepare("UPDATE video_generations SET updatedAt = ? WHERE id = ?").run(ts, input.generationId);
  db.prepare(
    "UPDATE video_workspaces SET updatedAt = ? WHERE id = (SELECT workspaceId FROM video_generations WHERE id = ?)"
  ).run(ts, input.generationId);
  return run;
}

export function updateVideoGenerationStatus(input: {
  generationId: string;
  status: GenerationStatus;
  errorMessage?: string;
  warningMessage?: string;
  ...
  const db = getDb();
  const latestRun = db
    .prepare("SELECT id FROM video_generation_runs WHERE generationId = ? ORDER BY createdAt DESC LIMIT 1")
  ...
  db.prepare("UPDATE video_generation_runs SET status = ?, errorMessage = ?, warningMessage = ?, completedAt = ?, updatedAt = ? WHERE id = ?").run(
    input.status,
...
export function updateVideoGenerationRunStatus(input: {
  runId: string;
  status: GenerationStatus;
  errorMessage?: string;
  warningMessage?: string;
  ...
  const db = getDb();
  const row = db
    .prepare("SELECT generationId FROM video_generation_runs WHERE id = ?")
  ...
  db.prepare("UPDATE video_generations SET updatedAt = ? WHERE id = ?").run(ts, row.generationId);
  db.prepare(
    "UPDATE video_workspaces SET updatedAt = ? WHERE id = (SELECT workspaceId FROM video_generations WHERE id = ?)"
...
export async function appendVideoGenerationAssets(input: {
  generationId: string;
  runId: string;
  assets: AssetInput[];
}) {
  ...
  const existedCountRow = db
    .prepare("SELECT COUNT(1) AS c FROM asset_links WHERE ownerType = 'video_generation_run' AND ownerId = ?")
  ...
  const commitTx = db.transaction((rows: PendingInsert[]) => {
    ...
    for (const row of rows) {
      ...
      db.prepare(
        `INSERT INTO asset_links (id, assetId, ownerType, ownerId, role, sortOrder, createdAt, metaJson)
         VALUES (?, ?, 'video_generation_run', ?, 'output', ?, ?, ?)`
      ).run(
        row.linkId,
        row.assetId,
        input.runId,
    ...
    const ts = now();
    db.prepare("UPDATE video_generation_runs SET updatedAt = ? WHERE id = ?").run(ts, input.runId);
    db.prepare("UPDATE video_generations SET updatedAt = ? WHERE id = ?").run(ts, input.generationId);
    db.prepare(
      "UPDATE video_workspaces SET updatedAt = ? WHERE id = (SELECT workspaceId FROM video_generations WHERE id = ?)"
    ).run(ts, input.generationId);
...
export async function deleteVideoAsset(input: { linkId: string }) {
  const db = getDb();
  const deleteTx = db.transaction((linkId: string) => {
    const row = db
      .prepare(
        `SELECT l.assetId AS assetId, a.filePath AS filePath, r.id AS runId, r.generationId AS generationId, g.workspaceId AS workspaceId
         ...
         JOIN video_generation_runs r ON r.id = l.ownerId
         JOIN video_generations g ON g.id = r.generationId
         WHERE l.id = ? AND l.ownerType = 'video_generation_run'`
      )
    ...
    const ts = now();
    db.prepare("UPDATE video_generation_runs SET updatedAt = ? WHERE id = ?").run(ts, row.runId);
    db.prepare("UPDATE video_generations SET updatedAt = ? WHERE id = ?").run(ts, row.generationId);
    db.prepare("UPDATE video_workspaces SET updatedAt = ? WHERE id = ?").run(ts, row.workspaceId);
    return row;
  });
...
export async function deleteVideoGeneration(input: { generationId: string }) {
  const db = getDb();
  const deleteTx = db.transaction((generationId: string) => {
    const generation = db
      .prepare("SELECT workspaceId FROM video_generations WHERE id = ?")
    ...
    const assets = db
      .prepare(
        `SELECT DISTINCT l.assetId AS assetId, a.filePath AS filePath
         ...
         JOIN assets a ON a.id = l.assetId
         WHERE l.ownerType = 'video_generation_run'
           AND l.ownerId IN (SELECT id FROM video_generation_runs WHERE generationId = ?)`
      )
    ...
    db.prepare(
      `DELETE FROM asset_links
       WHERE ownerType = 'video_generation_run'
         AND ownerId IN (SELECT id FROM video_generation_runs WHERE generationId = ?)`
    ).run(generationId);
    db.prepare("DELETE FROM video_generation_runs WHERE generationId = ?").run(generationId);
    db.prepare("DELETE FROM video_generations WHERE id = ?").run(generationId);
    db.prepare("UPDATE video_workspaces SET updatedAt = ? WHERE id = ?").run(now(), generation.workspaceId);
    return assets;
  });
